...
import { KIND_COLOR, KIND_ORDER, FLOW_COLOR, topoRanks, inferFlow, resolveTokenColor as resolveVar } from './shared.jsx';
import { PaperLights, ShadowFloor, StudioEnv } from './lighting.jsx';
...
function Node({ n, pos, color, on, selected, onSelect, reduced }) {
  ...
  // Hover and selection read through the scale above and the label's ring below. Nothing
  // emits on paper (D68), and an off-step node is faded, so it casts no shadow either.
  return (
    <group>
      <mesh
        ref={ref}
      ...
      >
        <button
          ...
          /* NO `outline` here. This is a focusable <button>, and an inline style beats
             ...
             Selection is a solid ring and hover a 30% ring, never a glow: a label is a
             button, and the picked glow never sits under a button (D68). */
          style={{
            borderColor: color,
            opacity: on ? 1 : 0.3,
            boxShadow: selected
    ...
    <group>
      ...
      {on && !reduced && (
        <mesh ref={packet}>
          ...
          {/* A solid dot of the edge's own colour, moving along it. It used to be a
...
export default function Scene3D({ c, reduced }) {
  ...
  const palette = useMemo(() => {
    ...
    // hard-set to `color-scheme: light` (D63), there is not one `prefers-color-scheme` rule
    // in src/, and there is no theme switcher — so an empty dep array is CORRECT, not a bug.
