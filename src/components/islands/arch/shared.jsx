...
/**
 * Resolve a CSS colour — a `var(--token)` or any colour CSS accepts — to '#rrggbb' for
 * WebGL. Both 3D scenes read their colours through this ONE function.
 *
 * three's colour parser reads hex, rgb() and hsl() only. This site's tokens are oklch()
 * (D63), and a string three cannot parse leaves the material WHITE, with a console warning
 * nobody reads. That is how every 3D material here rendered from 2026-08-26 to 2026-09-24:
 * each scene handed the raw token value straight to three. Painting the colour into a 1×1
 * canvas and reading the pixel back lets the browser do the conversion it already knows.
 * A value the browser rejects WARNS and takes the fallback, never passes silently.
 */
let colourProbe = null;
export function resolveTokenColor(expr, fallback = '#888888') {
  let value = expr || '';
  ...
  if (!value) {
    console.warn(`[3d] colour ${expr} resolved to nothing; drawing it as ${fallback}.`);
    return fallback;
  }
  try {
    if (!colourProbe) {
      ...
      colourProbe = cv.getContext('2d', { willReadFrequently: true });
    }
    const SENTINEL = '#010203';
    colourProbe.fillStyle = SENTINEL;
    colourProbe.fillStyle = value; // an unparseable value leaves the sentinel in place
    if (colourProbe.fillStyle === SENTINEL && value.toLowerCase() !== SENTINEL) {
      console.warn(`[3d] the browser could not parse colour "${value}" (${expr}); drawing it as ${fallback}.`);
      return fallback;
    }
    colourProbe.clearRect(0, 0, 1, 1);
    colourProbe.fillRect(0, 0, 1, 1);
    const [r, g, b] = colourProbe.getImageData(0, 0, 1, 1).data;
    return `#${[r, g, b].map((x) => x.toString(16).padStart(2, '0')).join('')}`;
  } catch (err) {
    console.warn(`[3d] could not resolve colour ${expr}: ${err}; drawing it as ${fallback}.`);
    return fallback;
  }
}
 ...
 *
 * ponytail: derived depth, not authored depth. Upgrade path: populate the `z` field that
 * already exists in the schema (it is `null` on all 36 nodes today) from the private
 * Command Center OS records and re-run `npm run publish:data`. That needs the operator's machine (D2). This derives
 ...
 *
 * ponytail: heuristic, inferred from the kinds at each end. Upgrade path: author `flow`
 * in the private CCOS records (operator's machine, D2). An explicit value always
