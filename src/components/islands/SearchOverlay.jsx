              ...
              {(() => {
                ...
                return groups.map((g) => (
                  <li key={g.label} role="none">
                    {/* A listbox may only contain options and groups. The label was a bare
                        div with no role, i.e. a stray non-option child; the inner ul is now a
                        role="group" that the label names. HeaderSearch.jsx carries the exact
                        same block — the two are hand-parallel, so fix both or neither. */}
                    <div id={`ip-search-grp-${g.label.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}`} className="mono-label px-4 pb-1 pt-3">{g.label}</div>
                    <ul role="group" aria-labelledby={`ip-search-grp-${g.label.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}`}>
                      {g.items.map((rec) => {
                        flatIdx += 1;
                        const i = flatIdx;
