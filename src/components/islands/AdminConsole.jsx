...
function ResumeTab() {
  ...
  const [direction, setDirection] = useState(''); // tone / angle / emphasis — all three modes
  const [instruction, setInstruction] = useState(''); // what to change on a redraft
  const [history, setHistory] = useState([]); // prior drafts, newest last — powers Undo
  const [markdown, setMarkdown] = useState('');
  const [warnings, setWarnings] = useState([]);
  const [error, setError] = useState('');
  const [session, setSession] = useState(null);
  const [copied, setCopied] = useState(''); // '' | 'raw' | 'formatted' | 'plain'
  const [busy, setBusy] = useState(''); // '' | 'revise' | 'regenerate' — which redraft button is running

  /* One call for all three paths. `draft` present means REVISE (the server edits it);
     absent means generate fresh. The draft we send is the `markdown` STATE, which the
     result textarea writes to — so the operator's hand-edits go with it. */
  const callGenerate = async (extra = {}) => postJson('/api/admin/resume/generate', {
    mode: docMode, jobDescription: jd.trim(),
    notes: docMode === 'cover' ? notes.trim() : '',
  ...
  const generate = async () => {
    ...
    const r = await postJson('/api/admin/resume/generate', {
      mode, jobDescription: jd.trim(), notes: mode === 'cover' ? notes.trim() : '', direction: direction.trim(),
    });
  ...
  /* Redraft from the ready view. Deliberately does NOT reset — jd, notes and direction
     must survive or the next pass loses its grounding and its target role. `kind`
     'revise' edits the draft on screen; 'regenerate' folds the instruction into the
     direction and starts fresh. Both push the outgoing draft onto `history` first, so
     one bad instruction never costs a draft the operator liked. */
  const redraft = async (kind) => {
    if (!instruction.trim() || busy) return;
    setError('');
    setBusy(kind);
    const outgoing = markdown;
    const r = await callGenerate(kind === 'revise'
      ? { instruction: instruction.trim(), draft: markdown }
      : { direction: [direction.trim(), instruction.trim()].filter(Boolean).join('\n\n') });
    setBusy('');
  ...
  // Plain reads, no side effect inside a state updater — an updater can run twice.
  const undo = () => {
    if (!history.length) return;
    setMarkdown(history[history.length - 1]);
    setHistory(history.slice(0, -1));
    setWarnings([]); // the restored draft's warnings weren't kept; the lint is advisory
  };
  ...
  const reset = () => { setPhase('input'); setMode('refresh'); setDocMode('refresh'); setJd(''); setNotes(''); setDirection(''); setInstruction(''); setHistory([]); setBusy(''); setMarkdown(''); setWarnings([]); setError(''); setSession(null); };
  ...
  if (phase === 'ready') {
    return (
      <div className="flex flex-col gap-5">
        ...
        {/* Adjust after reading. Revise edits the draft above (hand-edits included);
            Regenerate starts over with this folded into the direction. */}
        <div className="flex flex-col gap-3 rounded-xl border border-line bg-surface/60 p-4">
          <Field label="Adjust it" hint="a suggestion, a tone, an angle — then revise the draft above, or start over with it">
            <textarea
              rows={2}
              maxLength={2000}
              value={instruction}
              onChange={(e) => setInstruction(e.target.value)}
              placeholder={docMode === 'cover'
                ? 'e.g. warmer opener; cut the second paragraph; lead with the MRP story…'
                : 'e.g. cut Selected Projects to three; lead with supply-chain depth; shorter, one page…'}
              className={inputCls}
            />
          </Field>
          <div className="flex flex-wrap items-center gap-3">
            <button
              onClick={() => redraft('revise')}
              disabled={!instruction.trim() || !!busy}
              className="btn btn-primary disabled:opacity-50"
            >
              {busy === 'revise' ? 'Revising…' : 'Revise with this'}
            </button>
            <button
              onClick={() => redraft('regenerate')}
              disabled={!instruction.trim() || !!busy}
            ...
            {history.length > 0 && (
              <button onClick={undo} disabled={!!busy} className="text-xs text-cyan-deep disabled:opacity-50">
                ↩ undo {history.length === 1 ? 'revision' : `revision (${history.length})`}
              </button>
            )}
          </div>
          <p className="text-xs text-ink-faint">
            <strong className="font-medium text-ink-muted">Revise</strong> edits the draft above and keeps everything
            else — including any edits you made in the box yourself. <strong className="font-medium text-ink-muted">Regenerate</strong>{' '}
            writes a new one from scratch with this added. Either way your facts stay bound to the canonical data.
          </p>
        </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Direction is a SEPARATE channel from Personal notes and available on every
          mode. Notes are WHAT to work in (facts, a referral); direction is HOW to
          write it (tone, angle, emphasis, length). The chips only append text into
          this same box, so there is one steering channel and it stays fully editable. */}
      <div className="flex flex-col gap-2">
        <Field label="Direction" hint="optional — tone, angle, emphasis, length. Steers how it is written, never what is true.">
          <textarea
            rows={3}
            maxLength={2000}
            value={direction}
            onChange={(e) => setDirection(e.target.value)}
            placeholder={mode === 'cover'
              ? 'e.g. warmer and shorter; lead with the supply-chain story rather than the AI work…'
              : 'e.g. lead with supply-chain depth; keep it to one page; foreground the orchestration work…'}
            className={inputCls}
          />
        </Field>
        {/* Outside the Field on purpose: Field renders a <label>, and interactive
            content inside a label is invalid and would steal the click to the textarea. */}
        <div className="flex flex-wrap gap-1.5">
          {(mode === 'cover'
            ? ['Warmer, less formal', 'Shorter — keep it tight', 'Lead with the supply-chain story', 'Lead with AI orchestration']
            : ['Lead with supply-chain depth', 'Lead with AI orchestration', 'Keep it to one page', 'Fewer projects, more depth', 'Plainer language']
          ).map((p) => (
            <button
              key={p}
              type="button"
...
function DataGroup({ status }) {
  ...
  return (
    <div className="space-y-5">
      <p className="text-sm leading-relaxed text-ink-muted">
        Repos pull from public APIs, and a scheduled action refreshes them daily. The job board's
        schedule is paused, so it refreshes only when you trigger it here. Tools, architectures, and cron read from your private
