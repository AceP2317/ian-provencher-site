...
import { PaperLights, ShadowFloor, StudioEnv } from '../arch/lighting.jsx';
// The one colour resolver both 3D scenes share: tokens are oklch(), which three cannot parse.
import { resolveTokenColor as resolveVar } from '../arch/shared.jsx';
  ...
  return (
    <Html position={[pos[0], pos[1] + 0.62, pos[2]]} center distanceFactor={9} zIndexRange={[20, 0]} style={{ pointerEvents: on ? 'auto' : 'none' }}>
      <button
        ...
        /* NO inline `outline`: an inline style beats `:focus-visible`, so the old
           `outline: none` left every label a keyboard target with no focus ring. The
           architecture scene fixed the same bug earlier; this copy had kept it. Selection
           is a solid ring and hover a 30% ring (the site's lift ring), never a glow,
           because a label is a button (D68). */
        style={{
          borderColor: color,
          opacity: on ? 1 : 0.28,
          boxShadow: selected
            ? `0 0 0 1px ${color}, var(--elev-1)`
            : hovered && on
              ? `0 0 0 1px color-mix(in oklab, ${color} 30%, transparent), var(--elev-1)`
              : 'var(--elev-1)',
        }}
      >
...
function Core({ pos, color, reduced, on, selected, onSelect, hovered, setHovered }) {
  ...
  return (
    ...
    >
      {/* Lit, not self-lit (D68): the core reads through its shaded facets and its
          shadow. The coloured point light that sat inside it went with the bloom. */}
      <mesh ref={inner} castShadow>
        <icosahedronGeometry args={[0.62, 0]} />
        <meshStandardMaterial color={color} roughness={0.35} metalness={0.2} flatShading />
      </mesh>
      <mesh ref={ring1} rotation={[Math.PI / 2.4, 0, 0]} castShadow>
        <torusGeometry args={[1.05, 0.03, 12, 60]} />
        <meshStandardMaterial color={color} roughness={0.4} metalness={0.2} />
      </mesh>
      <mesh ref={ring2} rotation={[0, 0, Math.PI / 3]}>
        <torusGeometry args={[1.3, 0.022, 12, 60]} />
        <meshStandardMaterial color={color} transparent opacity={0.8} roughness={0.4} metalness={0.2} />
      </mesh>
    </group>
  );
}
...
function Module({ station, pos, color, on, selected, onSelect, reduced }) {
  ...
  // Hover and selection read through the lift, the scale and the label's ring above.
  // Nothing emits on paper (D68), so there is no glow to raise.
  const hoverProps = {
    onPointerOver: (e) => { e.stopPropagation(); setHovered(true); },
    onPointerOut: () => setHovered(false),
  };
  ...
  return (
    <group>
      <group ref={ref} position={pos} onClick={(e) => { e.stopPropagation(); onSelect(); }} {...hoverProps}>
        {/* An off-step module is faded, so it casts no shadow: a full-strength shadow
            under a ghost would say the ghost is solid. */}
        {isOperator ? (
          <mesh castShadow={on}>
            <icosahedronGeometry args={[0.5, 0]} />
            <meshStandardMaterial color={color} transparent opacity={on ? 1 : 0.3} roughness={0.4} metalness={0.15} flatShading />
          </mesh>
        ) : (
          <RoundedBox args={size} radius={0.07} smoothness={4} steps={1} castShadow={on}>
            <meshStandardMaterial color={color} transparent opacity={on ? 1 : 0.28} roughness={0.45} metalness={0.15} />
          </RoundedBox>
        )}
      </group>
    ...
    <group>
      ...
      {on && !reduced && (
        <mesh ref={packet}>
          ...
          {/* A solid dot of the conduit's own colour, moving along it. It used to be a
              self-lit spark tuned for bloom, which washed out on the light ground. */}
...
export default function Scene3D({ c, reduced }) {
  ...
  const palette = useMemo(() => {
    ...
    return {
      accent,
      canvas: resolveVar('var(--color-canvas)', '#eff1f5'),
      edge: resolveVar('var(--color-ink-faint)', '#6b7280'),
      grid: resolveVar('var(--color-line)', '#d5dae2'),
    };
  ...
  // The outputs run takes its station's own scope colour, like every other conduit. It
  // used to ask for `palette.accent.green`, a key the palette never had, so the colour
  // was undefined; green is reserved for status on this site anyway (D16).
  const outStation = setup.stations.find((s) => s.id === 'outputs');
  if (out && eng) conduits.push({ a: eng, b: out, color: outStation ? colorOf(outStation) : palette.edge, on: stationOn('outputs'), key: 'out', phase: 0.5 });
  ...
  return (
    ...
    <div className="arch3d-canvas panel panel--flat">
      ...
      >
        ...
        <PaperLights sky={palette.canvas} ground={palette.grid} span={7} />
        <StudioEnv />
