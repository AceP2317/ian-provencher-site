...
export const doctrineVersion = 'v23.1';
...
export const hooks: Hook[] = [
  ...
  {
    event: 'SubagentStop',
    name: 'Routing cost ledger',
    icon: 'bar-chart',
    does: 'Every time a delegated sub-agent finishes, it reads that run’s own transcript and records what the routing lane actually cost — model and tokens, which the completion event itself does not carry.',
    why: 'Routing decisions get judged on measured spend rather than on intent. The ledger also has to outlive the transcripts it is derived from, which a later offline walk could never do.',
  },
];
...
export const stackPages = [
  ...
  { slug: 'doctrine', title: 'Operating doctrine', icon: 'compass', blurb: `${clauseCount(doctrineClauses.length)} behavioral clauses I run the model against — versioned (${doctrineName} ${doctrineVersion}), injected into every session.` },
  ...
  { slug: 'hooks', title: 'The hooks', icon: 'webhook', blurb: `${clauseCount(hooks.length)} Claude Code hooks that keep the doctrine and register live — and the whole setup reproducible.` },
