...
import derived from './stack.generated.json';
 ...
 *
 *  THE VERSION IS READ, NOT TYPED. It sat at 'v37.0' while the canonical header said
 *  v44.1, and the drift checker that spots exactly this printed the warning into a log
 *  nobody read for nineteen days. It now comes from stack.generated.json, which
 *  export-public.mjs fills from ~/nexus/NEXUS.md's own header on the daily local run.
 *  The CLAUSES below stay hand-written paraphrases — that half of D1 is untouched, and
 *  it is the half the public-safe boundary actually rests on. */
export const doctrineName = 'NEXUS';
export const doctrineVersion = derived.doctrineVersion ?? 'unknown';
 ...
 */
/**
 * BOTH OF THESE ARE NOW READ FROM THIS MACHINE, and the comment that used to sit here
 * is why. It read "Real total across all events. Derived nowhere — verified against
 * settings.json 2026-08-24" above `hookCount = 32`. By 2026-09-12 the real figures were
 * 42 commands across 8 events, and PICKUP.md had predicted that drift verbatim. A value
 * documented as hand-verified is a value nobody re-verifies; CLAUDE.md's own rule is
 * "Counts belong DERIVED, never typed", and this is where that rule was being broken.
 *
 * The count is two levels down in settings.json — each event holds MATCHERS, each
 * matcher holds its own hooks[] — so counting matchers gives 16, which is wrong in a way
 * that looks plausible. export-public.mjs walks both levels.
 */
export const hookEvents = (derived.hookEvents ?? []) as readonly string[];
export const hookCount = derived.hookCount ?? 0;

/** Skills and subagents on this machine. Counted by what Claude Code actually LOADS —
 *  a skill is a directory carrying a SKILL.md, a subagent is a flat .md file — not by
 *  what the folder happens to hold. Counting directories reported 62 skills and 0
 *  subagents; both were wrong, and the zero was indistinguishable from a missing folder. */
export const skillCount = derived.skillCount ?? 0;
export const agentCount = derived.agentCount ?? 0;
...
export const hooks: Hook[] = [
  {
    ...
    name: 'The injectors',
    icon: 'compass',
    // The count was typed as "five" beside a derived six (found 2026-09-25); it is read now.
    does: `Before the model reads my message, ${derived.perEvent?.UserPromptSubmit ?? 'several'} hooks add what the turn needs: which delegation lane fits, what has already been taught so it is not taught twice, the intent gate for a build, and the closing structure the answer has to end in.`,
  ...
  {
    event: 'PreToolUse',
    name: 'The guards, before the action',
    icon: 'shield',
  ...
  {
    event: 'PostToolUse',
    name: 'The checks, after it lands',
    icon: 'wrench',
  ...
  {
    event: 'SessionStart',
    name: 'Mostly watching the watchers',
    icon: 'radar',
  ...
  },
  {
    ...
    name: 'When the model underneath changes',
    icon: 'refresh-cw',
    does: 'Notices that the session is now running on a different model, records which one, and re-checks whether the assumptions the setup was built on still hold for it.',
    why: 'Rules written for one model are not automatically right for the next. The switch is the moment that becomes true, and it is the moment nobody remembers to check.',
  },
  {
    event: 'Stop',
    name: 'Before the turn is handed back',
    icon: 'flag',
    does: 'Harvests what the turn learned about the tool it ran inside, and checks the answer against the shape every answer here has to take.',
    why: 'The end of a turn is the last point where a missing section can still be added. After it, the only fix is a correction the reader has already had to notice.',
  },
];

/** Hooks per event, read from this machine. Never typed — the card headings used to
 *  carry these numbers as words, and five of six were wrong by 2026-09-12. */
export const hooksPerEvent = (derived.perEvent ?? {}) as Record<string, number>;
...
export const statusRows: StatusRow[] = [
  {
    tag: 'BILL',
    accent: 'accent',
    reads: 'Claude Code’s own running cost estimate for the session, drawn with a tilde because it is an estimate and not an invoice. Cents are cut rather than rounded, so it never reads higher than the figure it came from.',
  },
  {
    tag: 'EDITS',
    accent: 'cyan',
