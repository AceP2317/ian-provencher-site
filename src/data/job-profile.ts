...
export const jobProfile = {
  ...
  /** The DOMAIN track's search terms — supply-chain and operations roles, admitted only when
   *  the employer is an AI company (the board's second, separately-ranked section). */
  domainRoles: [
  ...
  minScore: 65,
  /** Same two knobs for the domain track, which is capped and floored independently. */
  keepTopDomain: 50,
  minScoreDomain: 65,
  /** And again for the pay-led supply-chain track — strictly supply-chain roles with no AI
   *  content, admitted on a DISCLOSED salary at or above minSalary and individual-contributor
   *  scope. The floor is lower than the other two on purpose: a pure-pay role sits in the
   *  "loosely inside the interest zone" band by construction, so 65 would publish nothing. */
  keepTopSupply: 25,
  minScoreSupply: 55,
} as const;
...
export const profileSummary =
  'AI orchestration and applied-AI roles — forward-deployed, solutions, implementation, enablement, and AI-operations work — plus supply-chain and operations roles at AI companies, where the operations depth is the qualification. Fully remote, hybrid in the Research Triangle, or commutable in coastal/eastern North Carolina. Aggregated from many sources and scored for fit.';
