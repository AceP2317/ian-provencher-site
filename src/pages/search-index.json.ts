...
export async function GET() {
  ...
  for (const c of cronJobs) {
    // An undescribed job has no public name, so it has nothing to be found by.
    if (!c.label) continue;
    records.push({
      id: `cron:${slugify(c.label)}${c.status === 'retired' ? '-retired' : ''}`,
      title: c.status === 'retired' ? `${c.label} (retired)` : c.label,
      type: 'cron',
      typeLabel: 'Cron',
      section: 'cron',
